...
const Moviedetailpage = () => {
  ...
  const { id } = useParams();
  const { data: ReviewData, isLoading: isReviewLoading, isError: isReviewError, error: ReviewError } = useReview(id);
      ...
      <div className='container'>
        ...
        <div className='cm'>
          ...
          <ul className='detailul'>
            ...
            <li className='detailli'>{movie.adult?'애기들 보면 안돼!':'전체이용가'}</li>
          </ul>
          <div className='title'><Badge pill bg="success" className='suc'>Budget</Badge> {numberWithCommas(movie.budget)}</div>
          <div className='title'><Badge pill bg="success" className='suc'>Release Date</Badge> {movie.release_date}</div>
          <div className='title'><Badge pill bg="success" className='suc'>Run Time</Badge> {movie.runtime}분</div>
        </div>
      </div>
        ...
        <div className='container-2'>
            ...
            <div>
              {expandedReviews.includes(index) ? review.content : `${review.content.slice(0, 400)}...`}
              <div>
